...
- 사용자가 지명(부산, 강남 등)을 말한 촬영지 질문이면 그 지명으로 geocode_place 를 먼저
  부르고, 받은 좌표를 넣어 find_filming_spots 를 부른다. 지명을 말했는데 좌표 없이
  find_filming_spots 를 부르지 않는다 — 전국 촬영지가 통째로 딸려 나와 지도가 엉킨다.
